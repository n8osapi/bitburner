...
export let main=ns=>{
	...
	let css=`<style id="scanCSS">.sc{white-space:pre;color:${theme.secondary};line-height:1.1;font-size:12px}.sc .s{cursor:pointer;text-decoration:underline;color:${theme.secondary}}.sc .f{color:${theme.errordark}}.sc .r{color:${theme.primary}}.sc .r.f{color:${theme.infolight}}.sc .s::before{content:"◉";color:${theme.errordark}}.sc .r::before{color:${theme.successdark}}</style>`;
	...
	ns.alert(`<div class="sc new">${addSc().trimStart()}</div>`);
  doc.querySelectorAll(".sc.new .s").forEach(q=>q.addEventListener('click',()=>tcommand(r[q.childNodes[0].nodeValue])));
