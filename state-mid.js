...
state.servers.forEach(s=>{
    s.hackTime=ns.getHackTime(s.hostname)         // 0.05 GB
    s.growTime=ns.getGrowTime(s.hostname)         // 0.05 GB
    s.weakenTime=ns.getWeakenTime(s.hostname)     // 0.05 GB
    s.hackAnalyze=ns.hackAnalyze(s.hostname)      // 1.00 GB
    s.hackChance=ns.hackAnalyzeChance(s.hostname) // 1.00 GB
    s.processes=ns.ps(s.hostname)                 // 0.02 GB
    }
)
