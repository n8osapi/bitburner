...
export async function main(ns) {
    ...
    while (true) {
        ...
        var moneyKeep = 0.2 * s.stocks["ME"].tick.total
        ...
        for (const stock of stocks) {
            ...
            if (position[2]) {
                ns.print('Short Position: ' + stock + ', ')
                sellPositions(stock);
            }
            buyPositions(stock);
        }
        ns.print('Short Cycle Complete');
        await ns.sleep(6000);
    }
    function buyPositions(stock) {
        ...
        if (forecast <= stockBuyPer && volPer <= stockVolPer) {
            if (playerMoney - moneyKeep > ns.stock.getPurchaseCost(stock,minSharePer, "Short")) {
                ...
                var bought = ns.stock.short(stock, shares);
                if (bought>0) {
                var m = x(shares * bought)
                ns.print('Short Bought: ' + x(shares) + ' of ' + stock + ' for ' + m)
                ns.print("Short Reserving:" + ns.nFormat(moneyKeep))
                post(ns,"/api/buy",{type:"short",shares:shares,symbol:stock,pricePer:bought,priceTot:(shares*bought),time:Date.now()})
    ...
    function sellPositions(stock) {
        var forecast = ns.stock.getForecast(stock);
        if (forecast > 0.5) {
            var price = ns.stock.sellShort(stock, position[2]);
            if (price>0){
            var m = x(position[2] * price)
            ns.print('Short Sold: '+ stock + ' for ' + m)
            ns.tprint('                                        Sold: '+ stock + ' for ' + m)
            ns.toast('Short Sold: '+ stock + ' for ' + m)
            post(ns,"/api/sell",{type:"short",shares:position[2],symbol:stock,cost:position[3],pricePer:price,priceTot:(position[2]*price),time:Date.now()})
